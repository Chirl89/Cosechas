...
# Obtener AIC y BIC para ARIMA
aic_arima = arima_fit.aic
bic_arima = arima_fit.bic
aic_bic_scores['ARIMA'] = (aic_arima, bic_arima)
...
# Obtener AIC y BIC para ARCH
aic_arch = arch_fit.aic
bic_arch = arch_fit.bic
aic_bic_scores['ARCH'] = (aic_arch, bic_arch)
...
y_test_pred_lstm = lstm_pred[:len(test_original)]  # Corregido: comparar con test, no con train
mae_lstm, mse_lstm, rmse_lstm, mape_lstm = calculate_errors(test_original, y_test_pred_lstm)
errors['LSTM'] = (mae_lstm, mse_lstm, rmse_lstm, mape_lstm)

# Calcular AIC y BIC aproximados para LSTM
n_params_lstm = model_lstm.count_params()
mse_lstm = mean_squared_error(test_original, y_test_pred_lstm)  # Corregido: comparar con test
n_samples_lstm = len(test_original)
log_likelihood_lstm = -n_samples_lstm / 2 * (math.log(2 * math.pi) + math.log(mse_lstm) + 1)
aic_lstm = 2 * n_params_lstm - 2 * log_likelihood_lstm
bic_lstm = n_params_lstm * math.log(n_samples_lstm) - 2 * log_likelihood_lstm
aic_bic_scores['LSTM'] = (aic_lstm, bic_lstm)
...
y_test_pred_rf = rf_pred[:len(test_original)]
mae_rf, mse_rf, rmse_rf, mape_rf = calculate_errors(test_original, y_test_pred_rf)
errors['Random Forest'] = (mae_rf, mse_rf, rmse_rf, mape_rf)

# Calcular AIC y BIC aproximados para Random Forest
n_params_rf = rf.n_estimators  # Aproximamos el número de parámetros por el número de árboles
mse_rf = mean_squared_error(test_original, y_test_pred_rf)
n_samples_rf = len(test_original)
log_likelihood_rf = -n_samples_rf / 2 * (math.log(2 * math.pi) + math.log(mse_rf) + 1)
aic_rf = 2 * n_params_rf - 2 * log_likelihood_rf
bic_rf = n_params_rf * math.log(n_samples_rf) - 2 * log_likelihood_rf
aic_bic_scores['Random Forest'] = (aic_rf, bic_rf)
...
y_test_pred_mlp = mlp_pred[:len(test_original)]
mae_mlp, mse_mlp, rmse_mlp, mape_mlp = calculate_errors(test_original, y_test_pred_mlp)
errors['Perceptron'] = (mae_mlp, mse_mlp, rmse_mlp, mape_mlp)

# Calcular AIC y BIC aproximados para MLP
n_params_mlp = sum([np.prod(layer.shape) for layer in mlp.coefs_]) + np.prod(mlp.intercepts_[0].shape)
mse_mlp = mean_squared_error(test_original, y_test_pred_mlp)
n_samples_mlp = len(test_original)
log_likelihood_mlp = -n_samples_mlp / 2 * (math.log(2 * math.pi) + math.log(mse_mlp) + 1)
aic_mlp = 2 * n_params_mlp - 2 * log_likelihood_mlp
bic_mlp = n_params_mlp * math.log(n_samples_mlp) - 2 * log_likelihood_mlp
aic_bic_scores['Perceptron'] = (aic_mlp, bic_mlp)
...
with pd.ExcelWriter('output/predicciones_modelos_con_intervalos_y_metricas.xlsx') as writer:
    ...
    # Guardar los AIC y BIC en una hoja separada
    df_aic_bic = pd.DataFrame(aic_bic_scores, index=['AIC', 'BIC']).T
    df_aic_bic.to_excel(writer, sheet_name='AIC_BIC')

print("Predicciones, intervalos de confianza, métricas de error y AIC/BIC guardados en 'output/predicciones_modelos_con_intervalos_y_metricas.xlsx'")
